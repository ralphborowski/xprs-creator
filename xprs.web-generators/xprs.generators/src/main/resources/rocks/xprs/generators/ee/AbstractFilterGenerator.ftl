...
import rocks.xprs.runtime.db.Range;<#if !model.inheritsFrom?? >
import rocks.xprs.runtime.db.PageableFilter;</#if><#list model.types as type><#if toCanonicalClassName(type) != "">
 ...
 */
public abstract class Abstract${model.name?cap_first}Filter extends <#if model.inheritsFrom?? >${toJavaType(model.inheritsFrom)}Filter<#else>PageableFilter</#if> {
