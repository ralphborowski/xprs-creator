 ...
 */
public class ToCanonicalFilterClassNameMethod implements TemplateMethodModelEx {
      ...
      if (arguments.get(0) instanceof StringModel) {
        ...
        if (wrappedObject instanceof Type) {
          ...
          if (argument instanceof Model) {
            return argument.getCanonicalName().replace("|", ".filter.") + "Filter";
          }

          return "";
        } else {
          throw new TemplateModelException("[toCanonicalFilterClassName] Argument is of type " + arguments.get(0).getClass().getCanonicalName() + ". Should be rocks.xprs.languages.xprsm.model.Type");
        }
      }
    }
    
    throw new TemplateModelException("[toCanonicalFilterClassName] Argument is missing.");
  }
