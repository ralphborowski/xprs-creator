 ...
 */
public class ToCanonicalDaoClassNameMethod implements TemplateMethodModelEx {
      ...
      if (arguments.get(0) instanceof StringModel) {
        ...
        if (wrappedObject instanceof Type) {
          ...
          if (argument instanceof Model) {
            return ToPluralMethod.toPlural(argument.getCanonicalName().replace("|", ".dao."));
          }

          return "";
        } else {
          throw new TemplateModelException("[toCanonicalDaoClassName] Argument is of type " + arguments.get(0).getClass().getCanonicalName() + ". Should be rocks.xprs.languages.xprsm.model.Type");
        }
      }
    }
    
    throw new TemplateModelException("[toCanonicalDaoClassName] Argument is missing.");
  }
